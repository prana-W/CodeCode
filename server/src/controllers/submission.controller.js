...
const createSubmission = asyncHandler(async (req, res) => {
    ...
    if (!problem_id || !language || !source_code) {
        throw new ApiError(statusCode.BAD_REQUEST, 'problem_id, language and source_code are required.');
    }
    ...
    if (!isAdmin && !isCreator && new Date() < new Date(context.contest_start_time)) {
    ...
    return res
        ...
        .json(new ApiResponse(statusCode.CREATED, 'Submission created successfully.', submission));
});
...
const getContestSubmissions = asyncHandler(async (req, res) => {
    ...
    const isCreator = probe.length > 0 && probe[0].contest_authored_by === req.userId;
...
const getSubmissionById = asyncHandler(async (req, res) => {
    ...
    if (!isAdmin && !isCreator && !isOwner) {
        throw new ApiError(statusCode.FORBIDDEN, 'You are not allowed to view this submission.');
    }
