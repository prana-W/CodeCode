...
const createContest = asyncHandler(async (req, res) => {
    if (req.role !== 'user') {
        throw new ApiError(statusCode.FORBIDDEN, 'Only users with role "user" can create contests.');
    }

    const { title, description, contest_start_time, contest_end_time, division } = req.body;

    if (!title || !contest_start_time || !contest_end_time || !division) {
        throw new ApiError(statusCode.BAD_REQUEST, 'title, contest_start_time, contest_end_time and division are required.');
    }

    if (![1, 2, 3, 4, 5].includes(Number(division))) {
        throw new ApiError(statusCode.BAD_REQUEST, 'division must be one of 1, 2, 3, 4, 5.');
    }

    if (new Date(contest_start_time) >= new Date(contest_end_time)) {
        throw new ApiError(statusCode.BAD_REQUEST, 'contest_start_time must be before contest_end_time.');
    }
    ...
    return res
        ...
        .json(new ApiResponse(statusCode.CREATED, 'Contest created successfully.', contest));
});
...
const updateContest = asyncHandler(async (req, res) => {
    ...
    if (contest.authored_by !== req.userId) {
        throw new ApiError(statusCode.FORBIDDEN, 'You are not the author of this contest.');
    }
    ...
    return res
        ...
        .json(new ApiResponse(statusCode.OK, 'Contest updated successfully.', updated));
});
...
const toggleVerifyContest = asyncHandler(async (req, res) => {
    ...
    return res
        .status(statusCode.OK)
        .json(new ApiResponse(statusCode.OK, `Contest ${newStatus ? 'verified' : 'unverified'} successfully.`, updated));
});
