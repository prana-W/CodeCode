...
2. Creates a `BullMQ Worker` subscribed to `"submission-queue"`.
3. Waits idle; processes jobs one by one as they arrive.
...
- `fs.mkdtemp(os.tmpdir(), 'judge_')` creates an isolated temp dir, e.g. `/tmp/judge_aXb3/`
- Source file is written using the language-specific name:
...
### Step 2 — Build `run.sh`
`buildRunScript(language, timeoutSecs)` generates a shell script written to `/tmp/judge_aXb3/run.sh`:
...
```sh
#!/bin/sh
timeout 2 python3 /code/main.py
exit $?
```
...
docker run --rm \
  ...
  --cpus=1 \                  # 1 CPU core
  -v /tmp/judge_aXb3:/code \  # Mount temp dir into container as /code
  -i \                        # Pipe stdin (input_data)
...
`input_data` is piped as **stdin** to the container. Node.js enforces an outer timeout of `time_limit_ms + 10_000ms` (10s buffer for Docker startup overhead).

### Step 4 — Exit code → Verdict mapping

| Exit code | Source | Verdict |
|-----------|--------|---------|
| `0` | Success | Compare stdout with `expected_output` → `accepted` or `wrong_answer` |
...
### Step 5 — Cleanup
The `finally` block always runs `fs.rm(tmpDir, { recursive: true })` — temp files are deleted regardless of outcome.

### Return value
```js
{ verdict: 'accepted', execution_time_ms: 312 }
```
...
Judge Service (src/services/judge.js)
    │
    ├── Write source file + run.sh to /tmp/judge_XXX/
    ├── docker run (--network=none, --memory, --cpus, -v, stdin)
    │       └── Inside container:
    │               ├── compile (if C/C++/Java)
    │               └── timeout N ./binary  < stdin
